...
import getOrder from '../../lib/getSku';
...
type Params = {
  params: {
    ...
    // Add the _wc_points_logged_redemption field
    meta_data: {
      id: number;
      key: string;
      value: {
                  ...
                  <div className='pt-9'>
                    <div>
                      <div className="grid grid-cols-3">
                        ...
                        <div className="text-base font-light text-tkgray">
                          <p>{orderDetails && orderDetails.meta_data.map((meta) => {
        if (meta.key === '_wc_points_redeemed') {
          const pointsRedeemed = parseInt(meta.value);
          return (
            <div key={meta.id}>
              <p className="text-lg text-tkgray font-bold">Royalty Burned</p>
              {isNaN(pointsRedeemed) ? (
                <div>No Points Redeemed</div>
              ) : (
                <div>Points Redeemed: {pointsRedeemed}</div>
              )}
            </div>
          );
        } else if (meta.key === 'is_vat_exempt') {
          // Handle the 'is_vat_exempt' key if needed
        }
        // Add other conditions for different meta_data keys if needed
        return null;
      })}
       </p>
                          <p className="mt-2 text-lg text-tkgray font-bold">Royalty Account</p>
                          <p>{orderDetails.trader_account_code}</p>
