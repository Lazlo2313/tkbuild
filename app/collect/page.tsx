...
import getAllOrders from '../lib/getData';
      ...
      <table className="container mx-auto bg-white">
        <thead className="bg-gray-100">
          <tr className="text-left">
            ...
            <th className="hidden md:table-cell py-2 px-4 border-b border-gray-200">Click & Collect</th>
          </tr>
        </thead>
        <tbody>
          {currentProducts.map((product: Product) => (
            <tr key={product.id}>
              <td className="py-2 px-4 border-b border-gray-200 underline underline-offset-4 hover:text-green-700 text-bold">
                <Link href={`/collect/${product.id}`}>{product.number}</Link>
              </td>
              ...
              <td className=" md:table-cell py-2 px-4 border-b border-gray-200">R {product.total}</td>
              <td className="py-2 px-4 border-b border-gray-200">
                {product.shipping_lines.map((shippingLine) => {
                  const storeNameMeta = shippingLine.meta_data.find((meta) => meta.key === '_pickup_location_name');
                  return storeNameMeta ? <span key={storeNameMeta.id}>{storeNameMeta.value}</span> : null;
                })}
              </td>
            </tr>
          ))}
