...
import getData from '../lib/getBabwReport'
...
export default function Reports() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setOrderBeingPickedData(response.find((status) => status.slug === 'confirmed-order'));
  ...
  return (
    <div>
      <h1 className='mt-10 text-2xl font-bold'>Build A Bear Orders</h1>
      {loading ? (
        <p>Fetching data...</p>
      ) : (
        <>
